...
const schema = yup.object().shape({
	firstName: yup.string().required('Поле обязательно для заполнения').matches(/^([^0-9]*)$/, 'Имя может содержать только буквы'),
	...
	password: yup.string().required('Поле обязательно для заполнения').matches(/^([^а-яА-Я]*)$/, 'Только латиница').matches(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).{6,}$/, ' Пароль не соответствует требованиям'),
